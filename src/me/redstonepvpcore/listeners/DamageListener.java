...
public class DamageListener implements Listener {
	...
	public void register() {
		...
		bleedItemStack = ItemStackReader.fromConfigurationSection(config.getConfigurationSection("item"), 
		...
		dropChance = config.getInt("drop-chance");
	...
	public void onEntityDamage(EntityDamageByEntityEvent e) {
		...
		int[] customEnchantments = getCustomEnchantments(itemInHand);
		...
		if(getRandom(1, 100) <= dropChance) {	
			damager.getWorld().dropItemNaturally(target.getLocation().add(getRandom(-1, 1), 0, getRandom(-1, 1)),
					bleedItemStack).getItemStack().setAmount(getRandom(minimumAmount, maximumAmount));
		}
	}
