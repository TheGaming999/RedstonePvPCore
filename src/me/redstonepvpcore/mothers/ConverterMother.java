...
public class ConverterMother {
	...
	public void setup() {
		...
		Record record = new Record(XSound.matchXSound(redstoneUseSoundSection.getString("name")).orElse(null), null,
				null, (float) redstoneUseSoundSection.getDouble("volume"),
				(float) redstoneUseSoundSection.getDouble("pitch"), redstoneUseSoundSection.getBoolean("3d", false));
		if (record.sound != null) redstoneUseRecord = record;
		record = new Record(XSound.matchXSound(goldUseSoundSection.getString("name")).orElse(null), null, null,
				(float) goldUseSoundSection.getDouble("volume"), (float) goldUseSoundSection.getDouble("pitch"),
				goldUseSoundSection.getBoolean("3d", false));
		if (record.sound != null) goldUseRecord = record;
		record = new Record(XSound.matchXSound(emeraldUseSoundSection.getString("name")).orElse(null), null, null,
				(float) emeraldUseSoundSection.getDouble("volume"), (float) emeraldUseSoundSection.getDouble("pitch"),
				emeraldUseSoundSection.getBoolean("3d", false));
		if (record.sound != null) emeraldUseRecord = record;
		redstoneTakeItemStack = ItemStackReader.fromConfigurationSection(
				config.getConfigurationSection("redstone.take-item"), "material", "amount", "data", "name", "lore",
				"enchantments", "flags", " ");
		...
		emeraldTakeItemStack = ItemStackReader.fromConfigurationSection(
				config.getConfigurationSection("emerald.take-item"), "material", "amount", "data", "name", "lore",
				"enchantments", "flags", " ");
		redstoneGiveItemStack = ItemStackReader.fromConfigurationSection(
				config.getConfigurationSection("redstone.give-item"), "material", "amount", "data", "name", "lore",
				"enchantments", "flags", " ");
		goldGiveItemStack = ItemStackReader.fromConfigurationSection(config.getConfigurationSection("gold.give-item"),
				"material", "amount", "data", "name", "lore", "enchantments", "flags", " ");
		emeraldGiveItemStack = ItemStackReader.fromConfigurationSection(
				config.getConfigurationSection("emerald.give-item"), "material", "amount", "data", "name", "lore",
				"enchantments", "flags", " ");
	}
