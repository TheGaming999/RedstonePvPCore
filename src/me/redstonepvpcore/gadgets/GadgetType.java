...
public enum GadgetType {	
	...
	RANDOM_BOX, DROP_PARTY_ACTIVATOR, EXP_SIGN, FRAME_GIVER, REPAIR_ANVIL, CONVERTER;
	
	public final static Set<String> GADGET_TYPES = ImmutableSet.copyOf(Arrays.stream(values())
			.map(Enum::name)
			.collect(Collectors.toSet()));
	
	@Nullable
	public static GadgetType parseType(String dataHolderTypeName) {
		return GADGET_TYPES.contains(dataHolderTypeName.toUpperCase()) ? GadgetType.valueOf(dataHolderTypeName.toUpperCase()) : null;
	}
